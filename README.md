...
自动搜索在约 1 秒的初次收集后先显示可靠歌词，并继续查找更好的版本。可靠结果取得后即写入临时缓存，切歌返回或重启可立即复用，同时继续补搜；完成搜索后再确认最终版本。某来源已找到准确且满足当前逐字、双语偏好的版本时，停止该来源的额外查询；最高优先级来源满足条件后，提前结束自动搜索。加载期间显示“正在加载歌词”或“正在加载更多版本”；超时保留已有结果，重试时也保留同一查询的上次结果。

逐字标签依据实际可用的时间标记；允许零时长字符、未标记的标点和重叠时间，不会因为其中一个异常字符移除整句动画。普通 LRC 不会凭空生成逐字时间。网易云逐字歌词也会合并对应翻译。

## 安装

从 [Releases](https://github.com/wzk112/LyricsX/releases) 下载最新的 `LyricsX-2.0.9.zip`，解压后将 `LyricsX.app` 拖到 `/Applications`。
...
完整的安装、签名和版本说明见 [`docs/releases/v2.0.9.md`](docs/releases/v2.0.9.md)。
