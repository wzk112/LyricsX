...
- 悬浮窗提供 Liquid Glass 和“磨砂阅读”两个可点击图例，透明度可在 20–80% 之间调整，磨砂程度可单独调节，并保留锁定、解锁、点击穿透、鼠标悬停显示、隐藏和自由拖动。
...
长音辉光按单个时间片段的长度渐进增强：超过约 0.45 秒开始具备辉光，约 1 秒的慢唱已有可见效果，约 1.6 秒达到完整强度；不会把辉光固定在最后一个字母。HDR 辉光增强默认开启，亮度默认 1.6×，可在设置调到 1.0×–4.0×；手动关闭后会记住选择，升级也保留已保存的开关值。普通亮度下的辉光同样增强，无需打开 HDR 才能看见长音效果。
...
可见的活动歌词使用绑定各自原生窗口的显示刷新回调，根据当前屏幕最高刷新率直接采样播放时钟，并在窗口交互循环中继续更新。暂停、隐藏或没有活动时间片段时停止连续绘制。文字排版按内容、字号、环境和可用尺寸缓存，位置、缩放和辉光变化不重新测量整窗。逐字的连续刷新只作用于主歌词，翻译与下一句复用静态排版。快速行级歌词会缩短切换动画并加密行边界检查；主窗口隐藏时冻结列表选中行和进度控件，再次显示时恢复当前进度。
...
从 [Releases](https://github.com/wzk112/LyricsX/releases) 下载最新的 `LyricsX-2.0.20.zip`，解压后将 `LyricsX.app` 拖到 `/Applications`。
...
完整的安装、签名和版本说明见 [`docs/releases/v2.0.20.md`](docs/releases/v2.0.20.md)。
