...
悬浮窗使用一个当前歌词绘制层，旧句不等待移除动画完成；下一句沿用相同的换行排版，从预览位置平滑上移、放大。主歌词为两行时，为翻译和下一句分别保留空间。2.0.13 修复主窗口歌词成为当前句时向左跳动的问题，活动行和未活动行持续复用同一文字视图；快速切歌、隐藏窗口或打断缩放时，旧动画不会干扰新的状态。
...
换句、切歌、应用新歌词以及紧凑信息卡片切换时，悬浮窗内容使用非线性的轻微模糊过渡，玻璃外框持续复用。逐字提亮和连续追加继续使用局部动效，高速换句会缩短整体过渡，避免每个字符反复模糊整窗。

## 安装

从 [Releases](https://github.com/wzk112/LyricsX/releases) 下载最新的 `LyricsX-2.0.13.zip`，解压后将 `LyricsX.app` 拖到 `/Applications`。
...
完整的安装、签名和版本说明见 [`docs/releases/v2.0.13.md`](docs/releases/v2.0.13.md)。
